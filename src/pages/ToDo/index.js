...
const ToDoList = () => {
  ...
  const taskStatus = (e) => {
    if (e) {
      setTaskList(todos.filter((item) => item.suspended == e));
    } else {
      setTaskList(todos);
    }
  };
    ...
    function filterItems(arr, query) {
      return arr.filter(function (el) {
        return (
          ...
          el.domain.toLowerCase().indexOf(query.toLowerCase()) !== -1
        );
      });
    }
  ...
  const taskSort = (e) => {
    if (e === 'uid') {
      setTaskList(todos.sort((a, b) => a.uid - b.uid));
    } else {
      setTaskList(
        todos.sort((a, b) => {
            ...
            <Row className='g-4 mb-3'>
              <Col lg={6}>
                ...
                <Input
                  ...
                  onChange={validation.handleChange}
                  onBlur={validation.handleBlur}
                  value={validation.values.suspended || ''}
                >
                ...
                {validation.touched.suspended && validation.errors.suspended ? (
                  <FormFeedback type='invalid'>
